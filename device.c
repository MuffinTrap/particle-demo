...
static const char *sync_track_path(const char* patpfix,
		const char *base, const char *name)
{
	static char temp[FILENAME_MAX];
	strncpy(temp, patpfix, sizeof(temp) - 1);
...
static int read_track_data(struct sync_device *d, struct sync_track *t)
{
	...
	for (int j = 0; pfixes[j]; ++j) {
		fp = d->io_cb.open(sync_track_path(pfixes[j], d->base, t->name), "rb");
		if (fp) break;
	}
	if (!fp)
		return -1;
